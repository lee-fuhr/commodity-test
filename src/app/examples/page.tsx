    ...
    <main className="min-h-screen">
      ...
      <section className="py-8 px-6">
        <div className="max-w-4xl mx-auto space-y-16">
          {examples.map((example, i) => (
            <div key={i} className="bg-white rounded-xl border border-brand-200 overflow-hidden">
              {/* Industry header */}
              <div className="bg-brand-100 px-6 py-4 border-b border-brand-200">
              ...
              <div className="grid md:grid-cols-2">
                {/* Before */}
                <div className="p-6 border-b md:border-b-0 md:border-r border-brand-200">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-sm font-semibold text-brand-500 uppercase tracking-wide">Before</h3>
                    <ScoreBadge score={example.before.score} type="before" />
                  </div>

                  <blockquote className="text-xl font-medium text-brand-800 mb-6 border-l-4 border-score-critical pl-4">
                    &ldquo;{example.before.headline}&rdquo;
                  </blockquote>

                  <div>
                    <p className="text-sm font-semibold text-brand-700 mb-2">Issues detected:</p>
                    <ul className="space-y-2">
                      {example.before.issues.map((issue, j) => (
                        <li key={j} className="flex items-start gap-2 text-sm text-brand-600">
                          <svg className="w-4 h-4 text-score-critical mt-0.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                {/* After */}
                <div className="p-6 bg-score-excellent/5">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-sm font-semibold text-brand-500 uppercase tracking-wide">After</h3>
                    <ScoreBadge score={example.after.score} type="after" />
                  </div>

                  <blockquote className="text-xl font-medium text-brand-800 mb-6 border-l-4 border-score-excellent pl-4">
                    &ldquo;{example.after.headline}&rdquo;
                  </blockquote>

                  <div>
                    <p className="text-sm font-semibold text-brand-700 mb-2">What improved:</p>
                    <ul className="space-y-2">
                      {example.after.improvements.map((improvement, j) => (
                        <li key={j} className="flex items-start gap-2 text-sm text-brand-600">
                          <svg className="w-4 h-4 text-score-excellent mt-0.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <section className="py-16 px-6 bg-brand-50">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-h1 font-bold text-brand-900 text-center mb-8">
            The pattern is clear
          </h2>

          <div className="grid md:grid-cols-2 gap-8">
            <div className="bg-white rounded-lg p-6 border border-brand-200">
              <h3 className="text-lg font-semibold text-score-critical mb-4">
                Commodity messaging
              </h3>
            ...
            <div className="bg-white rounded-lg p-6 border border-brand-200">
              <h3 className="text-lg font-semibold text-score-excellent mb-4">
